...
```{r}
ggplot(mtcars,aes(x=qsec, y=disp, color=factor(cyl)))+
  geom_point() + scale_colour_brewer(palette = "Set1") +
  theme_bw() + facet_wrap(~am)
  xlab("sec") + 
  ylab("availability") + 
  ggtitle("MT  CARS")
  # Facet wrap - split into different sections according to the value, here am - 0 and 1
```
```{r}
ggplot(mtcars,aes(x=qsec, y=disp, color=factor(am)))+
  geom_point() + scale_colour_brewer(palette = "Set1") +
  theme_bw() + 
